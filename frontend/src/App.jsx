...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",direction:"rtl",background:C.bg,position:"relative"}}>
      <style>{CSS}</style>
          ...
          isGuest
            ...
            : <div key="profile-content" style={{animation:"fadeUp .2s ease"}}>
                <div style={{background:C.white,borderRadius:18,border:`1px solid ${C.border}`,padding:"22px 16px",marginBottom:12,textAlign:"center"}}>
                  ...
                  {user?.bio
                    ? <div onClick={()=>{setBioText(user.bio);setEditBio(true);}} style={{fontSize:13,color:C.muted,fontStyle:"italic",marginBottom:6,cursor:"pointer"}}>"{user.bio}"</div>
                    : <button onClick={()=>{setBioText("");setEditBio(true);}} style={{fontSize:11,color:C.indigo,background:"none",border:"none",cursor:"pointer",padding:"2px 0 6px",fontStyle:"italic"}}>✏️ כמה מילים עלי</button>}
        ...
        {TABS.map(([id,ic,lb])=>{
          ...
          return (
            <button key={id} onClick={()=>setTab(id)}
              ...
              {id==="profile"
                ? (user?.avatar
                    ? <div style={{width:a?26:22,height:a?26:22,borderRadius:"50%",overflow:"hidden",border:a?`2px solid ${C.ink}`:"2px solid #ccc"}}><img src={user.avatar} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/></div>
                    : <span style={{fontSize:a?24:21}}>👤</span>)
...
function UserProfile({ userId, onBack, BASE, C, HDR, SPINES, onViewBook }) {
  ...
  const API = BASE||"https://pageturner-production-5baf.up.railway.app";
  ...
  return (
    <div style={{animation:"fadeUp .2s ease",minHeight:"100%"}}>
      {/* Header */}
      <div style={{background:HDR,padding:"16px 16px 24px",marginBottom:0,position:"relative"}}>
        <button onClick={onBack} style={{background:"rgba(255,255,255,.15)",border:"none",cursor:"pointer",fontSize:16,color:"#fff",padding:"6px 12px",borderRadius:20,marginBottom:16,display:"flex",alignItems:"center",gap:4}}>← חזרה</button>
        <div style={{textAlign:"center"}}>
          <div style={{width:90,height:90,borderRadius:"50%",background:"rgba(255,255,255,.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:36,margin:"0 auto 12px",overflow:"hidden",border:"3px solid rgba(255,255,255,.3)"}}>
            {profile?.avatar ? <img src={profile.avatar} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/> : (profile?.type==="store"?"🏪":"👤")}
          </div>
          <div style={{fontSize:20,fontWeight:800,color:"#fff",marginBottom:4}}>{profile?.storeName||profile?.name||"..."}</div>
          {profile?.bio && <div style={{fontSize:13,color:"rgba(255,255,255,.7)",fontStyle:"italic",marginBottom:4}}>"{profile.bio}"</div>}
          {profile?.address && <div style={{fontSize:12,color:"rgba(255,255,255,.5)"}}>📍 {profile.address}</div>}
        </div>
        {/* Stats */}
        <div style={{display:"flex",justifyContent:"center",gap:32,marginTop:16}}>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:18,fontWeight:800,color:"#fff"}}>{books.length}</div>
      ...
      <div style={{padding:"16px"}}>
        {loading
          ...
          : books.length===0
          ? <div style={{textAlign:"center",padding:40,color:"#999",fontSize:14}}>אין ספרים זמינים</div>
          : <>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:4,marginBottom:12}}>
              {displayed.map(b=>(
                <div key={b.id} onClick={()=>onViewBook(b)} style={{aspectRatio:"2/3",borderRadius:8,overflow:"hidden",background:SPINES[parseInt(b.id)%SPINES.length]||"#888",cursor:"pointer",position:"relative"}}>
            ...
            {books.length>6 && !showAll && (
              <button onClick={()=>setShowAll(true)} style={{width:"100%",padding:"10px",background:"none",border:`1px solid ${C.border}`,borderRadius:10,fontSize:13,fontWeight:600,cursor:"pointer",color:C.ink}}>
...
function UserPage({ userId, onClose, onViewBook, C, HDR, SPINES }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:550,background:"red",overflowY:"auto",direction:"rtl"}}>
      {/* Header */}
      ...
      <div style={{marginRight:11}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingTop:9,borderTop:`1px solid ${C.border}`,marginTop:9}}>
          <div style={{display:"flex",alignItems:"center",gap:7}}>
            <div onClick={e=>{e.stopPropagation();e.preventDefault();alert("click! "+book.ownerid);if(book.ownerid&&onViewUser)onViewUser(book.ownerid);}} style={{width:28,height:28,borderRadius:"50%",background:color,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:11,overflow:"hidden",cursor:"pointer"}}>{book.owneravatar ? <img src={book.owneravatar} alt="" style={{width:"100%",height:"100%",objectFit:"cover",pointerEvents:"none"}}/> : (book.ownername||book.ownerName||"?")[0]}</div>
            <div>
