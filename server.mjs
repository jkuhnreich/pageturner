...
app.post("/api/auth/google", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      user = { id: crypto.randomUUID(), name: g.name || g.email.split("@")[0], email: g.email.toLowerCase(), avatar: g.picture || null, phone: "", type: "private", plan: "free", xp: 0, bookCount: 0, chatCount: 0, createdAt: Date.now(), authMethod: "google" };
      users.push(user);
      console.log("new google user:", user.name);
    }
    res.json({ ok: true, user, isNew: true });
  } catch(e) {
